...
<%
	...
	list.sort((a, b) -> {
		...
		return Integer.parseInt(b.select(".cumulative").text().replace(",", "").replace("명","").trim())
								- Integer.parseInt(a.select(".cumulative").text().replace(",", "").replace("명","").trim());
	...
	<table>
		...
		<% 
			...
			for(Element tr : list){
				...
				String name = tr.select(".title > a").text();
